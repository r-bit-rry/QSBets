...
news_df = pd.DataFrame([json.loads(item) for item in news])
st.write(news_df)

# Fetch and display press releases
st.header(f"Press Releases for {symbol}")
press_releases = fetch_stock_press_releases(symbol)
press_df = pd.DataFrame([json.loads(item) for item in press_releases])
st.write(press_df)
...
if indicators:
    ...
    if 'sma_20' in indicators:
        fig.add_trace(go.Scatter(x=x_range, y=indicators['sma_20'], mode='lines', 
                              name='SMA 20', line=dict(color='blue')), row=1, col=1)
    if 'sma_50' in indicators:
        fig.add_trace(go.Scatter(x=x_range, y=indicators['sma_50'], mode='lines', 
                              name='SMA 50', line=dict(color='orange')), row=1, col=1)
    if 'sma_100' in indicators:
        fig.add_trace(go.Scatter(x=x_range, y=indicators['sma_100'], mode='lines', 
                              name='SMA 100', line=dict(color='green', dash='dash')), row=1, col=1)
    ...
    fig.update_layout(
        height=1000,
        title_text=f"Technical Analysis Dashboard for {symbol}",
...
# Helper functions to find the most recent files
def find_latest_file(directory, pattern, symbol=None):
    ...
    # Sort files by date if they contain date in format YYYY-MM-DD
    files.sort(key=lambda x: x.split('_')[-1].split('.')[0] if '_' in x else '', reverse=True)
    
    if symbol:
        # First try to find symbol-specific files
...
def load_analysis_doc(symbol):
    ...
    if not files:
        ...
        # Sort by date if multiple files for the same symbol
...
def load_result_for_symbol(results_path, symbol):
    ...
    # Find the specific symbol result
...
# Load and display stock analysis and results
st.header("Stock Analysis Dashboard")

# First load the latest results file to get symbols for selection
...
available_symbols = [res.get("symbol") for res in results if res.get("symbol")]
if not available_symbols:
    available_symbols = ["AAPL"]  # Default if no symbols found
...
with tab2:
    ...
    analysis_data, file_name = load_analysis_doc(selected_symbol) or (None, None)
...
with tab3:
    ...
    if latest_results_file:
        ...
        if result_data:
            ...
            with col1:
                st.subheader("Entry Strategy")
                # Look for enter_strategy or enter_strategy with a space
                entry = None
                for key in result_data.keys():
                    if key.strip() == "enter_strategy":
                        entry = result_data[key]
                        break
                
                if entry is None:
                    entry = result_data.get("enter_strategy", {})
                ...
                if entry:
                    if isinstance(entry, dict):
                        ...
                        # If entry is a string, just display it directly
                        st.write(entry)
            
            with col2:
                st.subheader("Exit Strategy")
                # Look for exit_strategy or exit_strategy with a space
                exit_strategy = None
                for key in result_data.keys():
                    if key.strip() == "exit_strategy":
                        exit_strategy = result_data[key]
                        break
                
                if exit_strategy is None:
                    exit_strategy = result_data.get("exit_strategy", {})
                ...
                if exit_strategy:
                    if isinstance(exit_strategy, dict):
                        ...
                        # If exit_strategy is a string, just display it directly
                        st.write(exit_strategy)
        else:
            st.warning(f"No consultation results found for {selected_symbol}")
    else:
        st.error("No results files found")

# Comparison section (modified to use tabs)
...
if compare_symbols:
    ...
    compare_df = pd.DataFrame([{
        ...
        "Profit Target": res["exit_strategy"].get("profit_target", "N/A") if isinstance(res["exit_strategy"], dict) else "N/A",
        "Stop Loss": res["exit_strategy"].get("stop_loss", "N/A") if isinstance(res["exit_strategy"], dict) else "N/A",
        "Time Horizon": res["exit_strategy"].get("time_horizon", "N/A") if isinstance(res["exit_strategy"], dict) else "N/A"
